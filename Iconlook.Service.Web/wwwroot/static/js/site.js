            ...
            if (!selector.startsWith('cmd.on')) {
                ...
                if (message.cmd === 'BlockProducedSignal') {
                    ...
                    if ($('#block_grid .e-content tr').length > 1) {
                        ...
                        var block_row = $('#block_grid .e-content tr:first').clone().hide().css('opacity', 0);
                        ...
                        block_row.prependTo($('#block_grid .e-content tbody:first')).slideDown(150).animate({ opacity: 1 }, { queue: true, duration: 300 });
                    }
                    json.transactions.forEach(function(transaction) {
                        if ($('#transaction_grid .e-content tr').length > 1) {
                            ...
                            var trans_row = $('#transaction_grid .e-content tr:first').clone().hide().css('opacity', 0);
                            trans_row.find('.TransactionResponse_Amount').decimal(transaction.amount, 4, true);
                            trans_row.find('.TransactionResponse_To').text(transaction.to.substring(0, 10) + '..');
                            trans_row.find('.TransactionResponse_From').text(transaction.from.substring(0, 10) + '..');
                            trans_row.find('.TransactionResponse_Hash').text(transaction.hash.substring(0, 16) + '..');
                            trans_row.prependTo($('#transaction_grid .e-content tbody:first')).slideDown(150).animate({ opacity: 1 }, { queue: true, duration: 300 });
                        }
                    });
                    $('#block_grid .e-content tr').toArray().forEach(function(row, index) {
